...
+ There are five basic data types that we'll look at today - strings, integers, floats, booleans, and symbols.
...
+ I can use the Modulo (`%`) operator to tell me what the remainer would be. So this is a weird thing when you're starting out - there are actually some cool programming tricks that make this helpful, but for now this is something to be aware of.
...
+ This will actually return "1010". Why? Because both of those pieces of data are Strings, not numbers. Ruby actually doesn't know that they look like numbers to us. So instead of doing math addtion, the `+` operator for strings just takes two strings and smashes them together. This is more useful for things like "Hello " + "Ian" if you want to combine text.
